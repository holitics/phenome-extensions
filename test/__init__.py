...
from phenome.test.test_constants import *
...
class BaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.absolute_path_of_test_directory = rootdir + "/test"
        self.metadirectory = rootdir + "/phenome/config/meta/"

        if len(sys._unit_tests_app_name) > 0:
            self.app_metadata_directory = rootdir + "/" + sys._unit_tests_app_name + "/config/meta/"
        else:
    ...
    def _create_sensor_test_object(self, model_name='TEST_SENSOR'):
        ...
        self.assertTrue(sensor.actions.get('power_on') is not None)
